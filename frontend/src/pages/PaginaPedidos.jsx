...
import { TraerPedidos, VerDetallePedido, CambiarEstadoPedido, TraerProductos } from "../api/axios";
import "../styles/pedidos.css";

/* ══════════════════════════════════════════
   Constantes
══════════════════════════════════════════ */
const ESTADOS = ["en proceso", "confirmado", "enviado", "entregado", "cancelado"];

const ESTADO_META = {
  "en proceso":  { cls: "ped-badge--proceso",    icon: "🕐" },
...
══════════════════════════════════════════ */
function ModalDetalle({ pedido, userId, productos, onClose, onEstadoCambiado }) {
  ...
  const cargarDetalle = async () => {
    setLoading(true);
    try {
      const res = await VerDetallePedido({ id_usuario: userId, id_pedido: pedido.id });
      setDetalle(res.data);
    } catch {
  ...
  const precioProducto = (id) => productos.find((p) => p.id === id)?.precio ?? null;

  const handleGuardarEstado = async () => {
    if (estado === pedido.estado) return;
    setSaving(true);
    try {
      await CambiarEstadoPedido({ id_usuario: userId, id_pedido: pedido.id, estado });
    ...
    <div className="ped-overlay" onClick={onClose}>
      <div className="ped-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div>
              ...
              <div className="ped-modal__meta">
                ...
                {pedido.correocliente && (
                  <span className="ped-modal__email">✉️ {pedido.correocliente}</span>
                )}
                <span className="ped-modal__date">{formatDate(pedido.fecha_creacion)}</span>
              </div>
            </div>
          </div>
          <button className="ped-modal__close" onClick={onClose}>✕</button>
        </div>
        ...
        <div className="ped-modal__body">
          ...
          <div className="ped-section-label">Productos en este pedido</div>
          {loading ? (
            ...
            <p className="ped-empty-hint">Este pedido no tiene productos registrados.</p>
          ) : (
            <ul className="ped-product-list">
              {detalle.map((rel) => (
                <li key={rel.id} className="ped-product-row">
                  <div
          ...
          <div className="ped-estado-section">
            <p className="ped-section-label">Cambiar estado del pedido</p>
            ...
            <div className="ped-modal__actions">
              <button className="ped-btn ped-btn--ghost" onClick={onClose}>Cerrar</button>
              <button
      ...
      <div className="ped-page">
        ...
        <header className="ped-header">
          <div>
            ...
            <h1 className="ped-title">Gestión de <span>Pedidos</span></h1>
        ...
        <div className="ped-stats">
          {[
            ...
            { val: enProceso,      label: "En proceso", cls: "ped-stat--yellow" },
        ...
        {loading ? (
          ...
          <div className="ped-empty">
            <span className="ped-empty__icon">{pedidos.length === 0 ? "🚚" : "🔎"}</span>
            <p>
              {pedidos.length === 0
            ...
            {pedidos.length > 0 && (
              <button className="ped-btn ped-btn--ghost" onClick={() => { setSearch(""); setFilterEst("Todos"); }}>
                Limpiar filtros
              </button>
            )}
          </div>
              ...
              <tbody>
                {filtered.map((ped, i) => (
                  ...
                  >
                    ...
                    <td className="ped-table__total">{formatPrice(ped.totalcompra)}</td>
        ...
        {!loading && (
          <p className="ped-count">
            Mostrando <strong>{filtered.length}</strong> de <strong>{pedidos.length}</strong> pedidos
          </p>
        )}
      </div>
