...
function DescuentoBadge({ descuento }) {
  if (!descuento) return <span className="promo-badge promo-badge--off">Sin descuento</span>;
  return <span className="promo-badge promo-badge--discount">-{descuento}%</span>;
}

function EstadoBadge({ estado }) {
  return (
    <span className={`promo-badge ${estado ? "promo-badge--active" : "promo-badge--inactive"}`}>
...
function ModalEditarPromocion({ promo, userId, onClose, onGuardado }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      await ModificarPromocion({ id_usuario: userId, nombre, descripcion, descuento: Number(descuento) });
    ...
    <div className="promo-overlay" onClick={onClose}>
      <div className="promo-modal" onClick={(e) => e.stopPropagation()}>
        <div className="promo-modal__header">
          ...
          <button className="promo-modal__close" onClick={onClose}>✕</button>
        </div>
        <form className="promo-modal__form" onSubmit={handleSubmit}>
          <div className="promo-field">
            <label>Nombre <span className="promo-req">*</span></label>
            <input required value={nombre} onChange={(e) => setNombre(e.target.value)} placeholder="Ej: Rebajas de verano" />
          </div>
          <div className="promo-field">
            <label>Descripción</label>
            <textarea rows={2} value={descripcion} onChange={(e) => setDesc(e.target.value)} placeholder="Describe la promoción…" />
          </div>
          <div className="promo-field">
            <label>Descuento (%) <span className="promo-req">*</span></label>
            <div className="promo-input-suffix">
              <input type="number" min="0" max="100" required value={descuento} onChange={(e) => setDescuento(e.target.value)} />
              <span>%</span>
            </div>
            <div className="promo-discount-preview" style={{ "--pct": `${descuento}%` }}>
              <div className="promo-discount-preview__bar" />
              <span>{descuento}% de descuento</span>
            </div>
          </div>
          <div className="promo-modal__actions">
            <button type="button" className="promo-btn promo-btn--ghost" onClick={onClose}>Cancelar</button>
...
══════════════════════════════════════════ */
function ModalAgregarProductos({ userId, todosProductos, productosEnPromo, onClose, onGuardado }) {
  ...
  const confirmar = async () => {
    ...
    try {
      ...
      mostrarAlerta("success", `${sel.length} producto(s) agregado(s) a la promoción`);
      onGuardado();
      onClose();
    } catch (err) {
      mostrarAlerta("error", err?.response?.data?.detail ?? "Error al agregar productos");
    } finally { setSaving(false); }
  };
      ...
      <div className="promo-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="promo-modal__form">
          {disponibles.length === 0 ? (
            <p className="promo-empty-hint">Todos los productos ya están en esta promoción o tienen promoción unitaria.</p>
          ) : (
            <div className="promo-product-picker">
              {disponibles.map((p) => (
                <button
                  ...
                  onClick={() => toggle(p.id)}
                >
                  <span className="promo-product-chip__letter" style={{ "--hue": (p.id * 57) % 360 }}>
                    {p.nombre[0].toUpperCase()}
                  </span>
                  <span>{p.nombre}</span>
                  {sel.includes(p.id) && <span className="promo-product-chip__check">✓</span>}
                </button>
              ))}
            </div>
          )}
          {sel.length > 0 && <p className="promo-sel-count">{sel.length} seleccionado(s)</p>}
          <div className="promo-modal__actions">
            <button className="promo-btn promo-btn--ghost" onClick={onClose}>Cancelar</button>
            <button className="promo-btn promo-btn--primary" disabled={!sel.length || saving} onClick={confirmar}>
...
══════════════════════════════════════════ */
function ModalCrearUnitaria({ userId, todosProductos, promoUnitarias, onClose, onCreado }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      await CrearPromocionUnitaria({ id_usuario: userId, producto_id: Number(productoId), descuento: Number(descuento) });
  ...
  return (
    <div className="promo-overlay" onClick={onClose}>
      <div className="promo-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <form className="promo-modal__form" onSubmit={handleSubmit}>
          {disponibles.length === 0 ? (
            <p className="promo-empty-hint">Todos los productos ya tienen una promoción asignada.</p>
          ) : (
            <>
              <div className="promo-field">
                <label>Producto <span className="promo-req">*</span></label>
              ...
              <div className="promo-field">
                <label>Descuento (%) <span className="promo-req">*</span></label>
                <div className="promo-input-suffix">
                  <input type="number" min="1" max="100" required value={descuento} onChange={(e) => setDescuento(e.target.value)} />
                  <span>%</span>
                </div>
                <div className="promo-discount-preview" style={{ "--pct": `${descuento}%` }}>
                  <div className="promo-discount-preview__bar" />
...
══════════════════════════════════════════ */
function ModalEditarUnitaria({ promo, userId, nombreProducto, onClose, onGuardado }) {
  const [descuento, setDescuento] = useState(promo.descuento ?? 0);
  const [saving, setSaving]       = useState(false);
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      await ModificarPromocionUnitaria({ id_usuario: userId, id: promo.id_producto, descuento: Number(descuento) });
      mostrarAlerta("success", "Promoción actualizada");
      onGuardado();
      onClose();
    } catch (err) {
  ...
  return (
    <div className="promo-overlay" onClick={onClose}>
      <div className="promo-modal promo-modal--sm" onClick={(e) => e.stopPropagation()}>
        <div className="promo-modal__header">
          <div className="promo-modal__icon">✏️</div>
          <div>
...
function PaginaPromociones() {
  ...
  const cargar = async (id) => {
    setLoading(true);
    try {
      const [promoRes, prodPromoRes, unitariasRes, todosRes] = await Promise.allSettled([
        TraerPromocion(id),
      ...
      if (prodPromoRes.status === "fulfilled") setProdPromo(prodPromoRes.value.data);
      if (unitariasRes.status === "fulfilled") setUnitarias(unitariasRes.value.data);
  ...
  const togglePromoGeneral = async () => {
    try {
      ...
      reload();
      mostrarAlerta("success", `Promoción ${promo?.estado ? "desactivada" : "activada"}`);
    } catch (err) {
  ...
  const quitarProductoPromo = async (productoId) => {
    try {
      await EliminarProductoPromocion({ id_usuario: userId, producto_id: productoId });
      reload();
  ...
  const toggleUnitaria = async (idProducto) => {
    try {
      await CambiarEstadoPromocionUnitaria({ id_usuario: userId, id: idProducto });
      reload();
    } catch (err) {
      mostrarAlerta("error", err?.response?.data?.detail ?? "Error");
    }
      ...
      <div className="promo-page">
        ...
        <header className="promo-header">
          <div>
            ...
            <h1 className="promo-title">Gestión de <span>Promociones</span></h1>
            <p className="promo-subtitle">
              Administra descuentos generales para toda la tienda y promociones individuales por producto.
            </p>
          </div>
        </header>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="promo-stats">
              {[
                { val: promo ? 1 : 0,       label: "Promo general", cls: promo?.estado ? "promo-stat--green" : "" },
                { val: prodPromo.length,     label: "Productos en promo", cls: "promo-stat--blue" },
                { val: unitarias.length,     label: "Promos unitarias",   cls: "" },
                { val: unitActivas,          label: "Unitarias activas",  cls: "promo-stat--green" },
              ].map(({ val, label, cls }) => (
            ...
            <section className="promo-section">
              <div className="promo-section__header">
                <div className="promo-section__title-wrap">
                  ...
                  <h2 className="promo-section__title">Promoción general de la tienda</h2>
                </div>
                {promo && (
              ...
              {!promo ? (
                <div className="promo-empty-box">
                  <span>🏷️</span>
                  <p>Tu tienda aún no tiene una promoción general configurada.</p>
                  <p className="promo-empty-hint">La promoción general se crea automáticamente al registrar la tienda.</p>
                </div>
              ) : (
                <div className="promo-general-card">
                  ...
                  <div className="promo-general-card__info">
                    ...
                    {promo.descripcion && <p className="promo-general-card__desc">{promo.descripcion}</p>}
                  </div>
                  ...
                  <div className="promo-bar-wrap">
                    ...
                    <span className="promo-bar__label">{promo.descuento}% aplicado a todos los productos seleccionados</span>
                  </div>
                  ...
                  <div className="promo-prod-section">
                    <div className="promo-prod-section__bar">
                      <span className="promo-prod-section__label">
                        Productos incluidos
                        <span className="promo-prod-count">{prodPromo.length}</span>
                      </span>
                      <button
                    ...
                    {prodPromo.length === 0 ? (
                      <p className="promo-empty-hint">No hay productos asignados a esta promoción.</p>
                    ) : (
                        ...
                        {prodPromo.map((rel) => (
                          <div key={rel.id} className="promo-prod-chip">
                            ...
                            <button
                              ...
                              onClick={() => quitarProductoPromo(rel.producto_id)}
                            >
                              ✕
                            </button>
                          </div>
            ...
            <section className="promo-section">
              <div className="promo-section__header">
                <div className="promo-section__title-wrap">
                  ...
                  <h2 className="promo-section__title">Promociones por producto</h2>
                </div>
                <button
              ...
              {unitarias.length === 0 ? (
                ...
                <div className="promo-unitarias-grid">
                  {unitarias.map((u) => (
                    <div key={u.id} className={`promo-unitaria-card ${u.estado ? "promo-unitaria-card--active" : "promo-unitaria-card--inactive"}`}>
                      ...
                      <div className="promo-bar promo-bar--sm">
                        <div className="promo-bar__fill" style={{ width: `${u.descuento}%` }} />
                      </div>
                      ...
                      <div className="promo-unitaria-card__actions">
                        <button
                          ...
                          onClick={() => setModal({ type: "editarUnitaria", data: u })}
                        >
                          ✏️
                        </button>
                        <button
