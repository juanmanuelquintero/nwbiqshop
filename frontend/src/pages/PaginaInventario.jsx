...
function StockBadge({ stock }) {
  if (stock === 0)
    return <span className="inv-badge inv-badge--danger">Sin stock</span>;
  if (stock <= 5)
    return <span className="inv-badge inv-badge--warning">Bajo: {stock}</span>;
  return <span className="inv-badge inv-badge--success">{stock} uds.</span>;
}
function TipoBadge({ tipo }) {
  return (
    <span
      className={`inv-badge ${tipo === "variantes" ? "inv-badge--variant" : "inv-badge--simple"}`}
    >
    ...
    <div className="inv-img-wrapper">
      {tieneImagen ? (
        <div className="inv-img-preview">
          ...
          <button
            type="button"
            className="inv-img-preview__remove"
...
function ModalInventario({ producto, userId, onClose, onGuardado }) {
  ...
  const agregarTallaTemp = () => {
    ...
    if (nTallasTemp.find((t) => t.talla === nTalla)) {
      mostrarAlerta("error", `Ya tienes la talla ${nTalla}`);
      return;
    }
    setNTallasTemp((prev) => [
      ...prev,
      { talla: nTalla, cantidad: Number(nCant) },
    ]);
    setNCant("");
  };
  ...
  const agregarColor = async () => {
    ...
    try {
      ...
      mostrarAlerta(
        "error",
        err?.response?.data?.detail ?? "Error al agregar color",
      );
    } finally {
  ...
  const agregarTallaAColor = async (colorId) => {
    ...
    try {
      await AgregarTalla({
        ...
        id_color: colorId,
        talla: aTalla,
        cantidad: Number(aCant),
      });
      mostrarAlerta("success", `Talla ${aTalla} agregada`);
      setACant("");
      setAgregarTallaColorId(null);
      await cargarStock();
      onGuardado();
    } catch (err) {
      mostrarAlerta(
        "error",
        err?.response?.data?.detail ?? "Error al agregar talla",
      );
    }
  };
  ...
  const confirmarEliminar = async () => {
    ...
    try {
      if (eliminando.tipo === "color") {
        ...
        mostrarAlerta(
          "success",
          eliminando.tipo === "simple" ? "Eliminado" : "Talla eliminada",
        );
      }
      setEliminando(null);
      await cargarStock();
    ...
    setStockData((prev) =>
      prev.map((c, ci) =>
        ci !== colorIdx
          ...
          : {
              ...c,
              tallas: c.tallas.map((t, ti) =>
                ti !== tallaIdx ? t : { ...t, [key]: val },
              ),
            },
      ),
    );
  ...
  const stockTotal = esVariante
    ? stockData.reduce(
        (a, c) =>
          a + (c.tallas ?? []).reduce((b, t) => b + Number(t.cantidad ?? 0), 0),
        0,
      )
  ...
  return (
    <div className="inv-overlay" onClick={onClose}>
      <div
        className="inv-modal inv-modal--lg"
      ...
      >
        ...
        <div className="inv-modal__header">
          <div className="inv-modal__header-left">
            <div
              className="inv-modal__thumb"
              style={{ "--hue": (producto.id * 57) % 360 }}
            >
            ...
            <div>
              ...
              <div className="inv-modal__meta">
                ...
                <span className="inv-modal__price">
                  {formatPrice(producto.precio)}
                </span>
                <span className="inv-modal__total-stock">
                  Stock total:{" "}
                  <strong>{loadingStock ? "…" : stockTotal}</strong>
                </span>
              </div>
            </div>
          </div>
            ...
            <>
              ...
              {esVariante && (
                <>
                  <div className="inv-section-label">
                    <span>
                      ...
                      <InfoTrigger
                        onClick={() =>
                          setInfo(
                            "Las variantes permiten manejar un mismo producto con diferentes colores y tallas. Cada combinación puede tener su propio inventario.",
                          )
                        }
                      />
                    </span>
                    <button
                      ...
                      onClick={() => setMostrarAgregarColor((v) => !v)}
                    >
                    ...
                    <div className="inv-add-variante">
                      ...
                      <div className="inv-add-variante__grid inv-add-variante__grid--lg">
                        ...
                        <div className="inv-field">
                          ...
                          <input
                            ...
                            value={nMarca}
                            onChange={(e) => setNMarca(e.target.value)}
                          />
                        </div>
                        <div className="inv-field">
                        ...
                        <div className="inv-field">
                          <label>Imagen</label>
                          <InvImagenInput value={nImg} onChange={setNImg} />
                        </div>
                      </div>
                      <p style={{ fontWeight: 700, margin: "12px 0 6px" }}>
                        📐 Tallas para este color
                        <InfoTrigger
                          onClick={() =>
                            setInfo(
                              "Selecciona una talla y registra cuántas unidades tienes disponibles para este color.",
                            )
                          }
                        />
                      </p>
                      <div className="inv-add-variante__grid">
                        <div className="inv-field">
                          ...
                          <select
                            value={nTalla}
                            onChange={(e) => setNTalla(e.target.value)}
                          >
                            {TALLAS.map((t) => (
                        ...
                        <div className="inv-field">
                          <label>
                            ...
                            <InfoTrigger
                              onClick={() =>
                                setInfo(
                                  "Indica cuántas unidades disponibles tiene esta talla. Puedes usar cero si el producto está agotado.",
                                )
                              }
                            />
                          </label>
                      ...
                      <button
                        className="inv-btn inv-btn--add inv-btn--add-full"
                        style={{ marginTop: 12 }}
                        onClick={agregarColor}
                        disabled={addColorLoading || nTallasTemp.length === 0}
                      >
                        {addColorLoading
                          ? "Agregando…"
                          : `✓ Confirmar color "${nColor}"`}
                      </button>
                    </div>
                  )}
                  ...
                  {/* Lista de colores */}
                  {stockData.length === 0 ? (
                    <p className="inv-empty-msg">
                      Este producto no tiene colores registrados.
                    </p>
                  ) : (
                    stockData.map((colorFila, ci) => (
                      <div
                        key={colorFila.id_color}
                        className="inv-color-bloque"
                      >
                        {/* Header del color */}
                        <div className="inv-color-header">
                          <div className="inv-color-cell">
                            <span
                              ...
                              style={{
                                background: `hsl(${(colorFila.color?.length ?? 0) * 40}deg 55% 55%)`,
                              }}
                            />
                            {colorFila._editando ? (
                              <select
                                className="inv-inline-select"
                                value={colorFila.color ?? ""}
                                onChange={(e) =>
                                  setColorField(ci, "color", e.target.value)
                                }
                              >
                                {COLORES.map((c) => (
                                  <option key={c}>{c}</option>
                          ...
                          {colorFila._editando ? (
                            ...
                            >
                              <input
                                ...
                                placeholder="Marca"
                                value={colorFila.marca ?? ""}
                                onChange={(e) =>
                                  setColorField(ci, "marca", e.target.value)
                                }
                              />
                              <input
                                className="inv-inline-input"
                                placeholder="Referencia"
                                value={colorFila.referencia ?? ""}
                                onChange={(e) =>
                                  setColorField(
                                    ci,
                                    "referencia",
                              ...
                              <InvImagenInput
                                value={
                                  colorFila._imgFile ??
                                  (!colorFila._imgBorrada
                                    ? colorFila.imagen
                                    : null)
                                }
                                onChange={(f) => {
                                  if (f === null) {
                                    setColorField(ci, "_imgFile", null);
                                    setColorField(ci, "_imgBorrada", true);
                                  } else {
                                    setColorField(ci, "_imgFile", f);
                                    setColorField(ci, "_imgBorrada", false);
                                  }
                                }}
                              />
                              <button
                                className="inv-btn inv-btn--save"
                                onClick={() => guardarColor(ci)}
                                disabled={saving}
                              >
                                {saving ? "…" : "✓ Guardar color"}
                              </button>
                              <button
                                className="inv-btn inv-btn--cancel"
                                onClick={() => {
                                  setColorField(ci, "_editando", false);
                                  cargarStock();
                                }}
                              >
                                Cancelar
                            ...
                            >
                              {colorFila.imagen && (
                                <img
                                  src={colorFila.imagen}
                                  alt=""
                                  className="inv-variante-img"
                                />
                              )}
                              {colorFila.marca && (
                                <span className="inv-table__muted">
                                  🏷️ {colorFila.marca}
                                </span>
                              )}
                              {colorFila.referencia && (
                                <span className="inv-table__muted">
                                  🔖 {colorFila.referencia}
                                </span>
                              )}
                              <button
                                ...
                                title="Editar color"
                                onClick={() =>
                                  setColorField(ci, "_editando", true)
                                }
                              >
                                ✏️
                              </button>
                              {eliminando?.tipo === "color" &&
                              eliminando?.id === colorFila.id_color ? (
                                <>
                                  <span className="inv-confirm-text">
                                    ¿Eliminar color?
                                  </span>
                                  <button
                                ...
                                <button
                                  className="inv-icon-btn inv-icon-btn--del"
                                  title="Eliminar color"
                                  onClick={() =>
                                    setEliminando({
                                      tipo: "color",
                                      id: colorFila.id_color,
                                    })
                                  }
                                >
                                  🗑️
                        ...
                        <div className="inv-tallas-wrap">
                                ...
                                <tr>
                                  ...
                                  <th>
                                    ...
                                    <InfoTrigger
                                      onClick={() =>
                                        setInfo(
                                          "La cantidad representa las unidades disponibles de cada talla. Modifícala cuando recibas o vendas inventario.",
                                        )
                                      }
                                    />
                                  </th>
                          ...
                          {agregarTallaColorId === colorFila.id_color ? (
                            <div
                              className="inv-add-variante__grid"
                              style={{ marginTop: 8 }}
                            >
                              <div className="inv-field">
                                <label>Talla</label>
                                <select
                                  value={aTalla}
                                  onChange={(e) => setATalla(e.target.value)}
                                >
                                  {TALLAS.map((t) => (
                                    <option key={t}>{t}</option>
                                  ))}
                              ...
                              <div className="inv-field">
                                <label>
                                  ...
                                  <InfoTrigger
                                    onClick={() =>
                                      setInfo(
                                        "Escribe las unidades que vas a agregar para la nueva talla de este color.",
                                      )
                                    }
                                  />
                                </label>
                              ...
                              <button
                                className="inv-btn inv-btn--add"
                                onClick={() =>
                                  agregarTallaAColor(colorFila.id_color)
                                }
                                disabled={!aCant}
                              >
                                + Agregar
                            ...
                            <button
                              ...
                              onClick={() =>
                                setAgregarTallaColorId(colorFila.id_color)
                              }
                            >
                    ...
                    <div className="inv-simple-editor">
                      <div className="inv-simple-editor__img-row">
                        ...
                        {stockData[0]._editando ? (
                          <InvImagenInput
                            value={
                              stockData[0]._imgFile ??
                              (!stockData[0]._imgBorrada
                                ? stockData[0].imagen
                                : null)
                            }
                            onChange={(f) => {
                              if (f === null) {
                                setColorField(0, "_imgFile", null);
                                setColorField(0, "_imgBorrada", true);
                              } else {
                                setColorField(0, "_imgFile", f);
                                setColorField(0, "_imgBorrada", false);
                              }
                            }}
                          />
                        ) : stockData[0].imagen ? (
                          <img
                            src={stockData[0].imagen}
                            alt=""
                            className="inv-simple-img"
                          />
                        ) : (
                          <div className="inv-simple-img inv-simple-img--empty">
                            Sin imagen
                          </div>
                        )}
                      </div>
                      <div className="inv-simple-editor__current">
                        <p className="inv-simple-editor__label">
                          Cantidad actual
                        </p>
                        {!stockData[0]._editando && (
                          <>
                            <span className="inv-simple-editor__val">
                              {stockData[0].cantidad}
                            </span>
                            <StockBadge stock={Number(stockData[0].cantidad)} />
                          </>
                        )}
                      </div>
                      {stockData[0]._editando ? (
                        <div className="inv-simple-editor__form">
                          <div className="inv-simple-editor__form-row">
                            <div className="inv-field">
                              <label>
                                ...
                                <InfoTrigger
                                  onClick={() =>
                                    setInfo(
                                      "Actualiza la cantidad total disponible de este producto simple. El valor se refleja en el stock de tu tienda.",
                                    )
                                  }
                                />
                              </label>
                              <input
                                type="number"
                                min="0"
                                value={stockData[0].cantidad}
                                onChange={(e) =>
                                  setColorField(0, "cantidad", e.target.value)
                                }
                                autoFocus
                              />
                            </div>
                            <div className="inv-field">
                              ...
                              <input
                                placeholder="Opcional"
                                value={stockData[0].marca ?? ""}
                                onChange={(e) =>
                                  setColorField(0, "marca", e.target.value)
                                }
                              />
                            </div>
                            <div className="inv-field">
                              ...
                              <input
                                placeholder="Opcional"
                                value={stockData[0].referencia ?? ""}
                                onChange={(e) =>
                                  setColorField(0, "referencia", e.target.value)
                                }
                              />
                            </div>
                          </div>
                          <div className="inv-simple-editor__actions">
                            <button
                              className="inv-btn inv-btn--cancel"
                              onClick={() => {
                                setColorField(0, "_editando", false);
                                cargarStock();
                              }}
                            >
                              Cancelar
                        ...
                        <div className="inv-simple-editor__info-row">
                          {stockData[0].marca && (
                            <span className="inv-simple-tag">
                              🏷️ {stockData[0].marca}
                            </span>
                          )}
                          {stockData[0].referencia && (
                            <span className="inv-simple-tag">
                              🔖 {stockData[0].referencia}
                            </span>
                          )}
                          <button
                            className="inv-btn inv-btn--edit-simple"
                            onClick={() => setColorField(0, "_editando", true)}
                          >
                            ✏️ Modificar
                          </button>
                        </div>
