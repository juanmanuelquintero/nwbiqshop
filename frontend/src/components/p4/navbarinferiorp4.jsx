function NavbarInferior({ nombre, cantidadBolsa = 0, onAbrirBolsa }) {
  return (
    <div className="navbar-inferior">
      <label className="navbar-inferior__nombre">{nombre}</label>

      <button
        type="button"
        className="navbar-inferior__bolsa-btn"
        onClick={onAbrirBolsa}
        aria-label={`Ver bolsa${cantidadBolsa > 0 ? `, ${cantidadBolsa} productos` : ""}`}
      >
        {cantidadBolsa > 0 && (
          <span className="navbar-inferior__badge">{cantidadBolsa}</span>
        )}
        <svg
          width="28"
          height="28"
